...
const SelectDocType = () => {
	...
	return (
		<Menu as="div" className="relative inline-block w-full text-left mb-2">
			...
			<Menu.Button className="flex-shrink-0 justify-between w-full z-10 inline-flex items-center py-2.5 px-4 text-sm font-medium text-center text-zinc-500 bg-zinc-100 border border-zinc-300 rounded-xl hover:bg-zinc-200 focus:outline-none dark:bg-zinc-900 dark:hover:bg-zinc-800 dark:text-white dark:border-zinc-600">
				{DocType
					? DocType.name
						? DocType.name
			...
			>
				<Menu.Items className="absolute right-0 origin-top-right left-0 mt-2 w-full bg-white divide-y divide-zinc-100 rounded-xl shadow-2xl dark:bg-zinc-900 z-50 border-zinc-300 dark:border-zinc-600 max-h-64 overflow-y-auto scrollbar-thin scrollbar-thumb-zinc-300 scrollbar-track-transparent scrollbar-thumb-rounded-full scrollbar-track-rounded-full">
					<ul
						...
						{Documents.map((Document) => {
							return (
								<li key={Document.id}>
																...
																"Available" ? (
																	<button
																		...
																		className={classNames(
																			...
																			"inline-flex w-full px-4 py-2 text-sm rounded-xl"
																		)}
																		onClick={(
																			e
																		) => {
																	...
																	>
																		<div className="inline-flex items-center justify-start text-left gap-3">
																			<TableDocument
																				color="currentColor"
																				className="text-sky-600 text-9xl"
																				variant="Bulk"
																			/>
																			{
																				InnerDocument.name
                                    ...
                                    type="button"
                                    className={
                                      "inline-flex w-full px-4 py-2 text-sm rounded-xl text-zinc-400 cursor-not-allowed dark:text-zinc-600"
                                    }
                                    onClick={() =>
                                      toast.error(
